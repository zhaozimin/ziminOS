...
「记录灵感」不需要 QuickAdd。灵感集第一行是标题，Dataview 查询从第二行开始：打开笔记时能立即看到并直接勾选全部未完成任务，分组标题会同时显示这篇文件的真实最后更新时间，下方保留完整历史。设置 → ziminOS → 灵感收集里，可以改文件夹、笔记名称、定位标题、插入位置和单条格式；格式支持 `{{content}}`、`{{date}}`、`{{time}}`、`{{datetime}}`。v0.3.0 已生成的旧灵感集，下次记录灵感时会自动把系统查询移到标题下方，并补上最后更新分组。若想一键呼出，再到设置 → 快捷键给它绑定自己顺手的按键。
