 ...
 * [OUTPUT]: 对外提供 registerZiminosIcons，把二十四个图形注册进 Obsidian 的图标库，
 ...
 *        二十四个图形出自同一套画法（Pikaicons：24 格、2 宽描边、圆头圆角），
 *        其中十六个是 Pikaicons 免费集里的原图（MIT，见 docs/第三方组件.md），
 *        八个是照同一套画法补画的——它们要么在免费集里没有（暂停、播放、灯泡、调色板、对齐、循环），
 *        要么必须与兄弟图标成套才讲得清（季度那块四分之一饼），
 *        要么根本不属于任何命令（dock：设置页「边栏」标签页那块带左栏的面板）。
 *        补画的一律复用原图的几何原语（半径 9 的圆、日历框），
 ...
 * 学员在外观面板里把图标调细，只有我们这二十三个不跟着变的话，那条边栏会一眼看出是外来户。
