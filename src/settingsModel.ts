 ...
 * v0.17.0 的 editing 同理拿到自己的一页：粘贴与光标是两件互不相干的事，
 * 一个开关管不了两件。legacy 那三条**没有**页，判据同样是这一条的反面——
 ...
 * 最后四张管的不是笔记而是屏幕上的呈现（排版、编辑、文件、边栏）。
 * 这四张的先后是从「字」走到「屏幕」：排版管一篇笔记写成什么样，编辑管你打字时的手感，
...
export const TABS: readonly SettingsTab[] = [
    ...
    },
    {
        ...
        module: '编辑 v1',
        status: '运行中 · 选中文字粘一条网址就成外链；每篇笔记记住上次的光标位置',
    },
    {
        id: 'explorer',
        label: '文件',
 ...
 *
 * 用一个对象而不是六个位置参数：中间三个函数的类型都是 `() => void`，
