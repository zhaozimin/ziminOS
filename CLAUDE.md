...
<settings>
设置页按系统模块切成八张标签页（开荒 / 项目 / 灵感 / 复盘 / 人脉 / 客户 / 外观 / 边栏），页标识与 modules/ 下的目录同名，一页只回答一个系统的配置问题；目录名等设置也各自归还给它服务的那个模块，而不再堆在页尾一个统称「高级」的折叠区里——每页自己有一个，永远在页尾。八张页的身份（短名 / emoji / 模块全名 / 交付状态）收在 src/settings.ts 的 TABS 一张表里，标签栏与每页页头都从它出；模块清单不再单列一份，标签栏本身就是那份清单。加一页 = TABS 加一行 + panels 表加一个渲染函数，后者的 Record<TabId, …> 会在漏写时报编译错。
</settings>
